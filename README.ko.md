...
- `Esc` 또는 ×로 취소
- 앱 강제 전환·클립보드 사용 없음
- 상단 마이크 메뉴에서 녹음 제어, 앱에서 사용할 마이크 선택, 설정 열기
- 녹음 중 실제 입력 음량 표시, 단축키·시작/완료 소리·로그인 자동 실행 설정

> **macOS 개발자 프리뷰입니다.** Windows는 아직 지원하지 않습니다. 소스에서 설치하는 방식이며, 다른 컴퓨터에 바로 배포할 독립 실행형 DMG는 제공하지 않습니다. 모든 앱·입력칸의 호환성을 검증한 것은 아닙니다.

상세 기술·보안 문서는 영어로 관리합니다. 이 문서는 한국어 설치·사용 안내입니다. 현재 녹음 상태창의 문구도 한국어로 표시됩니다.
...
처음 뜨는 **AGY Dictation Voice Engine의 마이크 접근 요청**은 직접 허용하세요. 권한을 허용하기 전에 서비스가 멈췄다면 `start --apply`를 다시 실행합니다. 권한은 자동으로 허용되지 않습니다.
...
녹음 중에는 앱을 옮겨도 됩니다. **녹음을 끝낼 때 선택한 입력칸**에 글을 넣습니다. 종료 후 전사·입력이 끝나기 전 다른 곳을 클릭하거나 글자를 입력하면 자동 입력을 중단하고 전사문을 복구 파일에 보관합니다.

미국식 키보드 기준으로 Control + grave/backtick 또는 Control + backslash를 감지합니다. 한글 자판의 ₩ 표기는 키보드마다 위치가 다를 수 있습니다.

## 메뉴 막대와 설정

상단 **마이크 아이콘**을 누르면 현재 상태, 입력 장치, 녹음 시작·완료·취소,
설정, 음성 엔진 재시작과 종료 메뉴가 나옵니다. 녹음 중에는 아이콘이 빨간색으로
바뀌며 하단 HUD에 실제 입력 음량과 마이크 이름을 표시합니다.

- **마이크 선택:** Mac의 기본 마이크를 따르거나 특정 장치를 지정할 수 있습니다.
  다른 앱의 기본 마이크는 변경하지 않습니다. 지정한 장치가 끊기면 다른 마이크로
  자동 전환하지 않고 오류를 표시합니다. 장치 변경은 다음 녹음부터 적용됩니다.
- **입력 음량:** AGY로 보내는 오디오 데이터에서 계산합니다. 대기 중에는 마이크를
  열지 않습니다. 음량계는 소리가 들어오는지 확인하는 용도이며 전사 정확도를 보증하지 않습니다.
- **단축키:** 기본값은 `Ctrl + ₩`입니다. 단축키 표시나 연필을 클릭하고 원하는 키를
  직접 누른 다음 **저장** 또는 Enter를 누르세요. 키 조합, F키, 왼쪽·오른쪽 Command
  같은 보조 키 단독 지정이 가능합니다. Esc는 변경 취소, 휴지통은 단축키 끄기,
  되돌리기 아이콘은 기본값 복원입니다. 보조 키 단독 지정은 짧게 눌렀다 뗄 때
  작동하며, 다른 키·마우스와 함께 사용하면 녹음을 시작하지 않습니다.
- **소리:** 녹음 시작·완료 소리를 켜거나 끕니다.
- **자동 실행:** 설치된 서비스의 로그인 자동 실행을 켜거나 끕니다.

설정은 개인 데이터 폴더의 `settings.json`에 저장됩니다. 녹음·처리 중에는
마이크와 단축키 변경을 잠급니다. 마이크 선택과 음량계의 합성 데이터 검증은
통과했지만, 실제 발화부터 최종 입력까지의 검증은 별도로 필요합니다.

단축키를 지정하는 동안 전역 녹음 단축키는 잠시 멈춥니다. 다른 창으로 이동하거나
설정 창을 닫으면 저장하지 않은 변경을 취소합니다. 일반 글자·Space·Esc 단독과
앱 전환·Spotlight의 기본 조합은 등록하지 않습니다.
...
| 마이크 권한 안내 | AGY Dictation Voice Engine 마이크 허용 후 재시작 |
| 입력 위치가 바뀌었다는 안내 | 녹음 종료 후 전사·입력이 끝날 때까지 입력칸 유지 |
