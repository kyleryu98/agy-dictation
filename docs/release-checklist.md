...
현재 공개 저장소는 없으며 라이선스 결정도 보류 중입니다. 아래 항목은 앞으로 확인할 기준입니다. 체크되어 있지 않은 항목을 완료된 사실로 읽지 마세요.
...
- [ ] 소유자가 저장소 라이선스를 결정한다. 결정 전 임의의 오픈소스 라이선스를 붙이지 않는다.
- [ ] 공개 저장소를 실제로 만든 뒤 정확한 주소를 문서에 추가한다.
- [ ] 로컬 번들의 기존 Python 프레임워크·모듈 의존성을 명시한다.
- [ ] 독립 배포를 목표로 할 경우 의존성 패키징, 서명·공증, 새 Mac 실행 검증을 별도로 완료한다.
- [ ] 설치·자동 시작 등록·제거 절차를 별도로 검증한 뒤 공개한다.
- [ ] Windows는 실제 이식과 검증 전까지 미지원으로 표시한다.
